...
<style>
	...
	<tr> 	  
		...
		<td>${dto.writer }</td> 
		<td><fmt:formatDate value="${dto.regDate }" pattern="yyyy-MM-dd HH:mm:ss" timeZone="KST"/></td> 
		<td><fmt:formatDate value="${dto.updDate }" pattern="yyyy-MM-dd HH:mm:ss" timeZone="KST"/></td> 
...
<c:if test="${prev}">
 <span>[ <a href="list?num=${startPageNum - 1}">이전</a> ]</span>
</c:if>
...
<c:if test="${next}">
 <span> &nbsp; &nbsp;[ <a href="list?num=${endPageNum + 1}">다음</a> ]</span>
</c:if>
</div>
